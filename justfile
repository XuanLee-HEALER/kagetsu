...
play:
    ...
    cargo run --release --bin kagetsu --features dev-tools

# 别名: run = play
run: play

# Launcher debug 模式 (KAGETSU_DEBUG=1):
# 在 launcher 终端 stderr 打印 osascript 内容 + stderr, 排查 -10000 / 窗口位置问题.
# 用法:跑此 recipe → 看到 "[launcher/iTerm2] osascript script: ..." 之类输出.
play-debug:
    cargo build --release --bins --features dev-tools
    KAGETSU_DEBUG=1 cargo run --release --bin kagetsu --features dev-tools

# 开发模式 (debug 构建, 编译快但运行略慢)
dev:
    cargo build --bins --features dev-tools
    cargo run --bin kagetsu --features dev-tools

# 强制 inline 启动 (跳过 launcher 的新终端 spawn, 在当前终端跑)
play-inline:
    cargo build --release --bins --features dev-tools
    cargo run --release --bin kagetsu --features dev-tools -- --inline

# 直接跑游戏内核, 完全跳过 launcher (开发常用, 不开新窗口, 编译快)
play-game:
    cargo run --bin kagetsu-game --features dev-tools
