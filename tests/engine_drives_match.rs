...
use tui_majo::engine::match_state::{MatchState, match_apply};
use tui_majo::engine::op::{AtomicOp, OpError};
use tui_majo::engine::round_state::{
...
fn decide(state: &RoundState) -> AtomicOp {
    match state {
        ...
        RoundState::AwaitCalls(_) => {
            let ops = legal_ops(state);
            // 任何家能荣和就 Ron.
            for who in tui_majo::engine::domain::meld::Seat::ALL {
...
fn round_apply_rejects_illegal_op_for_phase() {
    ...
    let dummy_tile = tui_majo::engine::domain::tile::Tile {
        kind: tui_majo::engine::domain::tile::TileIndex(0),
        red: false,
        id: 0,
    };
