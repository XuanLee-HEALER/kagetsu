...
use kagetsu_core::net::protocol::{ClientMsg, RoomLifecycle, RoomView, ServerMsg};
...
impl OnlineRoomState {
        ...
        match msg {
            ...
            ServerMsg::RoomUpdate(view) => {
                ...
                if self.room_view.state == RoomLifecycle::InGame
                    && self.room_view.mode == kagetsu_core::net::p2p::RoomMode::Standard
                {
    ...
    pub fn render(
        ...
        f: &mut Frame,
        area: Rect,
        nat: Option<&kagetsu_core::net::p2p::host::NatReachability>,
        ...
        let (mode_label, mode_color) = match self.room_view.mode {
            kagetsu_core::net::p2p::RoomMode::Standard => ("Standard (房主权威)", Color::Cyan),
            kagetsu_core::net::p2p::RoomMode::ZeroTrust => {
        ...
        if self.is_host() {
            if let Some(reach) = nat {
                let (label, color) = match reach {
                    kagetsu_core::net::p2p::host::NatReachability::Public(_) => {
                        ("Public (公网可达, 加入者可直连)", Color::Green)
                    }
                    kagetsu_core::net::p2p::host::NatReachability::Private => {
                        ("Private (NAT 后, 加入者通过 relay 中转)", Color::Yellow)
                    }
                    kagetsu_core::net::p2p::host::NatReachability::Unknown => {
        ...
        let empty_label = match self.room_view.mode {
            kagetsu_core::net::p2p::RoomMode::Standard => "(开局补 AI)",
            kagetsu_core::net::p2p::RoomMode::ZeroTrust => "(等真人加入, ZeroTrust 不允许 AI)",
        };
        for i in 0..empty {
            lines.push(Line::from(Span::styled(
        ...
        let len_label = match cfg.length {
            kagetsu_core::engine::rules::LengthRule::Hanchan => "半庄战",
            kagetsu_core::engine::rules::LengthRule::Tonpuusen => "东风战",
        };
        ...
        let is_zerotrust = self.room_view.mode == kagetsu_core::net::p2p::RoomMode::ZeroTrust;
...
mod tests {
    ...
    use kagetsu_core::net::protocol::PlayerSlot;
    ...
    fn make_view(
        ...
        n_players: usize,
        state: RoomLifecycle,
        mode: kagetsu_core::net::p2p::RoomMode,
    ) -> RoomView {
    ...
    fn make_state(
        ...
        let view = make_view(
            ...
            n_players,
            RoomLifecycle::Lobby,
            kagetsu_core::net::p2p::RoomMode::Standard,
        );
    ...
    fn game_state_view_routes_to_enter_online_game() {
        ...
        use kagetsu_core::net::protocol::PlayerView;
        let (mut s, _, in_tx) = make_state(1, 1, 2);
        ...
        let view = kagetsu_core::net::protocol::GameStateView {
            round_wind: kagetsu_core::engine::round_state::RoundWind::East,
            kyoku: 1,
            honba: 0,
            riichi_sticks: 0,
            dealer: Seat::East,
            turn: Seat::East,
            phase: kagetsu_core::engine::phase::Phase::Draw,
            my_seat: Seat::East,
            my_hand: Vec::new(),
            my_last_drawn: None,
            players: [
    ...
    fn render_with_nat_and_dial_addr_does_not_panic() {
        ...
        let nat_pub = kagetsu_core::net::p2p::host::NatReachability::Public(
    ...
    fn render_with_zerotrust_short_humans_does_not_panic() {
        ...
        s.room_view.mode = kagetsu_core::net::p2p::RoomMode::ZeroTrust;
        let nat_priv = kagetsu_core::net::p2p::host::NatReachability::Private;
        let backend = ratatui::backend::TestBackend::new(144, 40);
        let mut term = ratatui::Terminal::new(backend).unwrap();
    ...
    fn render_with_nat_unknown_does_not_panic() {
        ...
        let nat_unknown = kagetsu_core::net::p2p::host::NatReachability::Unknown;
        let backend = ratatui::backend::TestBackend::new(144, 40);
        let mut term = ratatui::Terminal::new(backend).unwrap();
