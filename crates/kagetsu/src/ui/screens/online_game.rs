...
impl OnlineGameState {
    ...
    fn handle_msg(&mut self, msg: ServerMsg) -> Option<Transition> {
        match msg {
            ServerMsg::GameStateView(view) => {
                ...
                if !matches!(new_view.phase, kagetsu_core::engine::phase::Phase::AwaitCalls) {
    ...
    fn paint_my_status(
        ...
        let waits = if total == 13 {
            let counts = kagetsu_core::engine::domain::tile::count_by_kind(&view.my_hand);
            kagetsu_core::engine::domain::decompose::tenpai_tiles(&counts, &me.melds)
        } else if total == 14 {
            if let Some(drawn) = view.my_last_drawn {
                let mut counts = kagetsu_core::engine::domain::tile::count_by_kind(&view.my_hand);
...
fn meld_tiles(meld: &kagetsu_core::engine::domain::meld::Meld) -> Vec<Tile> {
    match &meld.kind {
...
mod tests {
    ...
    use kagetsu_core::net::protocol::{GameStateView, PlayerView};
    ...
    fn round_result_sets_message_and_clears_hints() {
        ...
        in_tx
            .send(ServerMsg::RoundResult(
                kagetsu_core::net::protocol::RoundResultView {
                    message: "流局".into(),
    ...
    fn game_end_sets_message_and_clears_hints() {
        let (mut s, _out_rx, in_tx) = make_state(1);
        s.current_hints = Some(vec![NetAction::Pon]);
        let dummy = kagetsu_core::engine::score::Ranking {
            seat: Seat::East,
            place: 1,
            raw_score: 25_000,
            return_diff_k: 0,
        ...
        in_tx
            .send(ServerMsg::GameEnd(kagetsu_core::net::protocol::GameOverView {
                rankings: [
                    dummy,
                    kagetsu_core::engine::score::Ranking {
                        seat: Seat::South,
                        place: 2,
                        ..dummy
                    },
    ...
    fn room_update_to_lobby_returns_main_menu_transition() {
        let (mut s, _out_rx, in_tx) = make_state(1);
        let view = kagetsu_core::net::protocol::RoomView {
            room_id: "x".into(),
            host_id: 1,
            config: kagetsu_core::engine::rules::GameRules::default(),
            players: Vec::new(),
