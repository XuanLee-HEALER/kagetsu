...
//!
//! 房间收到 [`kagetsu_core::net::protocol::ServerMsg::MpStart`] 后切到此屏. spawn 自己的
//! [`kagetsu_core::net::mp::actor::MpPlayerActor`] + [`kagetsu_core::net::p2p::mp_bridge::spawn_mp_bridge`],
//! 用 [`kagetsu_core::net::session::NetSession::mp_command_tx`] (走 SwarmTransport)
//! 跟 swarm 通信. 入站消息从 [`kagetsu_core::net::session::NetSession::mp_inbound_rx`]
...
use kagetsu_core::net::mp::MpPhase;
use kagetsu_core::net::mp::actor::{MpConfig, MpPlayerHandle, spawn_mp_player};
use kagetsu_core::net::mp::cmd::{MpEvent, MpRoomCmd};
use kagetsu_core::net::p2p::mp_bridge::{MpBridgeHandle, new_inbound_channel, spawn_mp_bridge};
use kagetsu_core::net::p2p::mp_swarm::SwarmTransport;
use kagetsu_core::net::session::NetSession;
use crate::ui::Transition;
use crate::ui::theme::{Theme, ThemeKind};

/// MpStart 参数 — 跟 [`kagetsu_core::net::protocol::ServerMsg::MpStart`] 字段一致.
#[derive(Debug, Clone)]
...
pub struct UiTable {
    ...
    pub own_melds: Vec<(kagetsu_core::mental_poker::wire::WireCallType, Vec<usize>)>,
...
impl ZeroTrustGameState {
    ...
    pub fn new(mut session: NetSession, args: MpStartArgs) -> Self {
        ...
        // 派生 mp_topic = kagetsu/mp/<session_label hex 前 16 字节>/v1
        // 4 方独立算一致, 因 session_label 来自 RoomActor 的统一计算.
        let topic_id = hex_short(&state.args.session_label);
        let mp_topic = format!("kagetsu/mp/{}/v1", topic_id);
    ...
    pub fn advance(&mut self) -> Option<Transition> {
        while let Some(msg) = self.session.try_recv() {
            if let kagetsu_core::net::protocol::ServerMsg::Error { message } = msg {
            ...
            MpEvent::ShouminkanApplied {
                ...
                if player == self.args.own_index {
                    ...
                    if let Some(m) = self.ui_table.own_melds.get_mut(target_meld_idx as usize) {
                        m.0 = kagetsu_core::mental_poker::wire::WireCallType::Kan;
    ...
    pub fn trigger_pon(&self) {
        self.trigger_call(kagetsu_core::mental_poker::wire::WireCallType::Pon, 2);
    }
    ...
    pub fn trigger_chi(&self) {
        self.trigger_call(kagetsu_core::mental_poker::wire::WireCallType::Chi, 2);
    }
    ...
    pub fn trigger_minkan(&self) {
        self.trigger_call(kagetsu_core::mental_poker::wire::WireCallType::Kan, 3);
    }

    /// 通用鸣牌触发 — own_drawn 取前 from_position 张 + last_discard.
    fn trigger_call(
        &self,
        call_type: kagetsu_core::mental_poker::wire::WireCallType,
    ...
    pub fn trigger_shouminkan(&self) {
        ...
        let target_idx = self
            ...
            .own_melds
            .iter()
            .position(|(ct, _)| *ct == kagetsu_core::mental_poker::wire::WireCallType::Pon);
    ...
    pub fn evaluate_yaku(&self) -> Option<YakuScoreSummary> {
        ...
        use kagetsu_core::engine::domain::decompose::decompose;
        use kagetsu_core::engine::domain::meld::{Meld, MeldKind, Seat};
        use kagetsu_core::engine::domain::tile::{Tile, TileIndex};
        use kagetsu_core::engine::domain::yaku::{WinContext, detect_yaku};
        use kagetsu_core::engine::rules::GameRules;
        use kagetsu_core::engine::score::evaluate;
        ...
        let melds: Vec<Meld> = self
            ...
            .filter_map(|(ct, ids)| {
                ...
                let kind = match ct {
                    kagetsu_core::mental_poker::wire::WireCallType::Pon if tiles_vec.len() == 3 => {
                        MeldKind::Pon {
                            tiles: [tiles_vec[0], tiles_vec[1], tiles_vec[2]],
                        }
                    }
                    kagetsu_core::mental_poker::wire::WireCallType::Chi if tiles_vec.len() == 3 => {
                        MeldKind::Chi {
                            tiles: [tiles_vec[0], tiles_vec[1], tiles_vec[2]],
                        }
                    }
                    kagetsu_core::mental_poker::wire::WireCallType::Kan if tiles_vec.len() == 4 => {
                        MeldKind::Minkan {
...
mod tests {
    ...
    async fn four_zerotrust_screens_keygen_and_shuffle_via_swarm() {
        use kagetsu_core::mental_poker::wire::MentalPokerMsg;
        use kagetsu_core::net::p2p::mp_swarm::SwarmCommand;
        use kagetsu_core::net::session::NetSession;
        use libp2p::PeerId;
        ...
        for i in 0..N {
            let (out_tx, _out_rx) = unbounded_channel::<kagetsu_core::net::protocol::ClientMsg>();
            let (_in_tx, in_rx) = unbounded_channel::<kagetsu_core::net::protocol::ServerMsg>();
    ...
    /// **M6.D 完整多回合 e2e**: 4 玩家轮流摸+弃 2 圈 (8 turn) → 验证:
    /// - 各家 discarded_pools 长度 = 2 (各家弃 2 张)
    /// - 4 screen 看到的 discarded_pools 一致 (gossipsub 全广播)
    /// - next_deck_index >= 8
    /// - own_drawn_in_hand 在每个 turn 后回 0
    #[tokio::test(flavor = "multi_thread", worker_threads = 4)]
    async fn zerotrust_4_players_two_rounds_consistent() {
        use kagetsu_core::mental_poker::wire::MentalPokerMsg;
        use kagetsu_core::net::p2p::mp_swarm::SwarmCommand;
        use kagetsu_core::net::session::NetSession;
        use libp2p::PeerId;
        ...
        let session_label = vec![0xD2u8; 32];
        let mut screens = Vec::with_capacity(N);
        for (i, sess) in sessions.into_iter().enumerate() {
            let args = MpStartArgs {
                all_peer_ids: peer_ids.iter().map(|p| p.to_bytes()).collect(),
        ...
        for round in 0..ROUNDS {
            for player in 0..N {
                ...
                assert_eq!(
                    screens[player].ui_table.own_drawn_in_hand.len(),
                    1,
                    "round {round} player {player} 应摸到 1 张"
                );

                screens[player].discard_cursor();
                let expected_pool_len = round + 1;
                let _ = tokio::time::timeout(Duration::from_secs(15), async {
                    loop {
                        for s in &mut screens {
                            let _ = s.advance();
                        ...
                        if screens
                            .iter()
                            .all(|s| s.ui_table.discarded_pools[player].len() == expected_pool_len)
                        {
                            return;
                        }
                        tokio::time::sleep(Duration::from_millis(20)).await;
                ...
                for (vi, s) in screens.iter().enumerate() {
                    assert_eq!(
                        s.ui_table.discarded_pools[player].len(),
                        expected_pool_len,
                        "round {round} screen {vi} player {player} 弃牌池长度应 {expected_pool_len}"
                    );
                }
                assert_eq!(
                    screens[player].ui_table.own_drawn_in_hand.len(),
                    0,
                    "round {round} player {player} 弃后回 0 张"
                );
            }
        }

        // Step 3: 验证 4 screen 看到的 discarded_pools 完全一致
        let reference: Vec<Vec<usize>> = screens[0].ui_table.discarded_pools.to_vec();
        for (vi, s) in screens.iter().enumerate() {
            for (player, ref_pool) in reference.iter().enumerate() {
                assert_eq!(
                    &s.ui_table.discarded_pools[player], ref_pool,
                    "screen {vi} player {player} pools 跟 screen[0] 不一致"
                );
            }
        }
        // 各家 ROUNDS 张
        for (player, ref_pool) in reference.iter().enumerate() {
            assert_eq!(ref_pool.len(), ROUNDS, "player {player} 应弃 {ROUNDS} 张");
        }

        // Step 4: next_deck_index >= 8
        for (vi, s) in screens.iter().enumerate() {
            assert!(
                s.next_deck_index >= (N * ROUNDS) as u32,
                "screen {vi} next_deck_index={} 应 >= {}",
                s.next_deck_index,
                N * ROUNDS
            );
        }
