...
use kagetsu_core::config::LoadResult;
use kagetsu_core::config::LocalPrefs;
...
pub struct App {
    ...
    pub host_listener: Option<kagetsu_core::net::p2p::host::HostHandle>,
    /// 房主 AutoNAT 探测出的可达性状态 (None = 还没探测).
    pub host_nat: Option<kagetsu_core::net::p2p::host::NatReachability>,
...
impl App {
    ...
    fn drain_host_events(&mut self) {
        use kagetsu_core::net::p2p::host::HostEvent;
    ...
    fn handle_key(&mut self, key: KeyEvent) -> Option<Transition> {
        ...
        if matches!(key.code, KeyCode::Char('q') | KeyCode::Char('Q')) {
            return Some(Transition::RequestConfirm {
                modal: Box::new(ConfirmModal::new("退出程序", "确定退出 kagetsu?")),
        ...
        match action {
            ...
            ConfirmAction::LeaveOnlineRoom | ConfirmAction::LeaveOnlineRoomViaEsc => {
                if let Screen::OnlineRoom(s) = &mut self.screen {
                    s.session.send(kagetsu_core::net::protocol::ClientMsg::Leave);
                }
                Transition::EnterMainMenu
            }
    ...
    fn apply_transition(&mut self, t: Transition) {
        match t {
            ...
            Transition::EnterOnlineLobby => {
                let bootstrap = kagetsu_core::net::p2p::bootstrap::effective_bootstrap_relays(
    ...
    fn create_online_room(&mut self, nickname: String, mode: kagetsu_core::net::p2p::RoomMode) {
        use kagetsu_core::net::p2p::bootstrap::{effective_bootstrap_relays, merge_relay_pool};
        use kagetsu_core::net::p2p::discovery::encode_metadata;
        use kagetsu_core::net::p2p::host::spawn_p2p_listener;
        use kagetsu_core::net::session::spawn_local_session;
        ...
        let lobby_meta = kagetsu_core::net::p2p::host::LobbyMeta {
        ...
        // 真正传到 RoomActor.
        use kagetsu_core::net::room::spawn_room_with_mode;
        ...
        let placeholder_view = kagetsu_core::net::protocol::RoomView {
            room_id: room_id_display,
            host_id: session.player_id,
            config: self.last_config.clone(),
            players: vec![],
            state: kagetsu_core::net::protocol::RoomLifecycle::Lobby,
            mode,
        };
    ...
    fn join_online_room(&mut self, nickname: String, addr: String) {
        use kagetsu_core::net::p2p::join::join_remote;
        ...
        match r {
            Ok(session) => {
                let placeholder_view = kagetsu_core::net::protocol::RoomView {
                    ...
                    mode: kagetsu_core::net::p2p::RoomMode::Standard,
                };
                let mut room_state = OnlineRoomState::new(session, placeholder_view);
                room_state.set_theme(self.local_prefs.theme);
                self.screen = Screen::OnlineRoom(Box::new(room_state));
