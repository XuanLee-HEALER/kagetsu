...
// `ThemeKind` 在 `kagetsu_core::config`(workspace 拆分前曾放在 ui::theme, 形成
// config → ui::theme → ThemeKind → config 反向耦合, Phase 0 解耦时上移).
// 此处 re-export 让旧引用路径 `crate::ui::theme::ThemeKind` 仍可用.
pub use kagetsu_core::config::ThemeKind;
