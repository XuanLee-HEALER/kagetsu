...
use kagetsu_core::ai::dummy::ai_choose_discard;
use kagetsu_core::ai::timeout::default_action_on_timeout;
use kagetsu_core::engine::domain::action::Action;
use kagetsu_core::engine::domain::meld::{MeldKind, Seat};
...
use kagetsu_core::engine::round_state::RoundResult;
use kagetsu_core::engine::rules::GameRules;
use kagetsu_core::engine::score::final_ranking;
use kagetsu_core::game_engine::{CallOptions, GameEngine};
...
impl GameScreenState {
    ...
    fn dev_quick_save(&mut self) {
        match kagetsu_core::dev::savestate::save(&self.engine, "quick") {
            Ok(path) => {
    ...
    fn dev_quick_load(&mut self) {
        match kagetsu_core::dev::savestate::load("quick") {
            Ok(g) => {
                self.engine = g;
    ...
    fn flush_recording_if_any(&mut self) {
        ...
        let rec = kagetsu_core::dev::recorder::RoundRecording {
        ...
        let _ = kagetsu_core::dev::recorder::save(&rec, &filename);
    }
    ...
    fn player(&self) -> &kagetsu_core::engine::player::PlayerState {
    ...
    fn highlight_kind(&self) -> Option<kagetsu_core::engine::domain::tile::TileIndex> {
        if !self.is_player_turn() || self.engine.phase() != Phase::AwaitDiscard {
            return None;
        }
    ...
    fn paint_my_status(&self, buf: &mut Buffer, ox: u16, oy: u16, theme: &Theme) {
        ...
        let waits = if total == 13 {
            kagetsu_core::engine::domain::decompose::tenpai_tiles(
                &kagetsu_core::engine::domain::tile::count_by_kind(&p.hand.closed),
                &p.hand.melds,
            )
        } else if total == 14 {
            // 摸完未切: 把 last_drawn 排除后剩 13 张算 tenpai.
            if let Some(drawn) = p.last_drawn {
                let mut counts = kagetsu_core::engine::domain::tile::count_by_kind(&p.hand.closed);
                counts[drawn.kind.0 as usize] = counts[drawn.kind.0 as usize].saturating_sub(1);
                kagetsu_core::engine::domain::decompose::tenpai_tiles(&counts, &p.hand.melds)
            } else {
...
fn riichi_index_in_river(p: &kagetsu_core::engine::player::PlayerState) -> Option<usize> {
    p.riichi_river_idx
}
