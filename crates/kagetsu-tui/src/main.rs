//! kagetsu launcher.
//!
//! 在合适的终端 emulator 中开新窗口跑 `kagetsu-game`. 找不到合适终端则 fallback
//! 到当前终端 inline 运行.
//!
//! ## 检测策略
//!
...
fn main() -> Result<()> {
    ...
    match try_launch_external(&game) {
        ...
        Err(e) => {
            eprintln!("[kagetsu] 外部终端启动失败 ({e}), fallback inline.");
            run_inline(&game)
        }
    }
}
...
fn locate_game_binary() -> Result<PathBuf> {
    ...
    if !path.exists() {
        bail!(
            "找不到游戏内核 binary: {}\n请确认已构建 kagetsu-game (cargo build --bin kagetsu-game)",
            path.display()
        );
    }
    Ok(path)
...
/// KAGETSU_DEBUG=1 时打印 script 内容供 debug.
#[cfg(target_os = "macos")]
fn run_osascript(script: &str, label: &str) -> Result<bool> {
    let debug = std::env::var("KAGETSU_DEBUG").is_ok();
    if debug {
