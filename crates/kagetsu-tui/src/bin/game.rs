//! kagetsu-game: 实际游戏内核 binary.
//!
//! 一般通过 launcher (kagetsu) 启动. 单独跑也可: `cargo run --bin kagetsu-game`.
...
fn main() -> Result<()> {
    ...
    let _ = execute!(stdout, SetTitle("kagetsu · 终端日麻"));
...
fn init_tracing() {
        ...
        .open(&log_path)
    {
        tracing_subscriber::fmt()
            ...
            .with_ansi(false)
            .init();
        tracing::info!("kagetsu started, log = {}", log_path.display());
    }
