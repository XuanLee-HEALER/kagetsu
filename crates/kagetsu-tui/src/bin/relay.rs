//! kagetsu-relay — 独立 libp2p relay 节点 (Tier 1 bootstrap).
//!
//! 部署到公网机器 (e.g. 阿里云国际版 ECS), 给 NAT 后的 kagetsu 客户端中转流量.
//! relay-server 不持有任何游戏状态, 也看不到加密流量内容 (libp2p Noise 端到端).
//!
//! 用法:
//! ```text
//! kagetsu-relay [PORT] [KEY_FILE] [EXTERNAL_IP]
//! kagetsu-relay                                  # 默认 PORT=4001 KEY=kagetsu-relay.key
//! kagetsu-relay 4001 ./relay.key 47.84.49.170    # 都自定
//! RUST_LOG=debug kagetsu-relay                   # 详细日志
//! ```
//!
...
use kagetsu_core::net::p2p::behaviour::{LOBBY_TOPIC, RELAYS_TOPIC};
...
const DEFAULT_KEY_FILE: &str = "kagetsu-relay.key";
        ...
        .with_behaviour(|key| {
            ...
            RelayBehaviour {
                identify: identify::Behaviour::new(
                    identify::Config::new("/kagetsu/id/1".into(), key.public())
                        .with_agent_version("kagetsu-relay/1".into()),
                ),
    ...
    if let Some(ip) = &external_ip {
        ...
        tracing::warn!(
            "未提供 EXTERNAL_IP — NAT 类云 (阿里云/AWS) 上 client 会报 NoAddressesInReservation. \
             启动加第三个参数 e.g. kagetsu-relay {port} {} <your-public-ip>",
            key_file.display()
        );
    }

    println!("=========================================================");
    println!("kagetsu-relay 启动");
