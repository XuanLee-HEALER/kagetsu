...
use majo_core::config::LoadResult;
use majo_core::config::LocalPrefs;
use majo_core::engine::rules::GameRules;
use majo_core::engine::score::Ranking;
...
pub struct App {
    ...
    pub host_listener: Option<majo_core::net::p2p::host::HostHandle>,
    /// 房主 AutoNAT 探测出的可达性状态 (None = 还没探测).
    pub host_nat: Option<majo_core::net::p2p::host::NatReachability>,
...
impl App {
    ...
    fn drain_host_events(&mut self) {
        use majo_core::net::p2p::host::HostEvent;
        ...
        match action {
            ...
            ConfirmAction::LeaveOnlineRoom | ConfirmAction::LeaveOnlineRoomViaEsc => {
                if let Screen::OnlineRoom(s) = &mut self.screen {
                    s.session.send(majo_core::net::protocol::ClientMsg::Leave);
                }
                Transition::EnterMainMenu
            }
    ...
    fn apply_transition(&mut self, t: Transition) {
        match t {
            ...
            Transition::EnterOnlineLobby => {
                let bootstrap = majo_core::net::p2p::bootstrap::effective_bootstrap_relays(
    ...
    fn create_online_room(&mut self, nickname: String, mode: majo_core::net::p2p::RoomMode) {
        use majo_core::net::p2p::bootstrap::{effective_bootstrap_relays, merge_relay_pool};
        use majo_core::net::p2p::discovery::encode_metadata;
        use majo_core::net::p2p::host::spawn_p2p_listener;
        use majo_core::net::session::spawn_local_session;
        ...
        let lobby_meta = majo_core::net::p2p::host::LobbyMeta {
        ...
        // 真正传到 RoomActor.
        use majo_core::net::room::spawn_room_with_mode;
        ...
        let placeholder_view = majo_core::net::protocol::RoomView {
            room_id: room_id_display,
            host_id: session.player_id,
            config: self.last_config.clone(),
            players: vec![],
            state: majo_core::net::protocol::RoomLifecycle::Lobby,
            mode,
        };
    ...
    fn join_online_room(&mut self, nickname: String, addr: String) {
        use majo_core::net::p2p::join::join_remote;
        ...
        match r {
            Ok(session) => {
                let placeholder_view = majo_core::net::protocol::RoomView {
                    ...
                    mode: majo_core::net::p2p::RoomMode::Standard,
                };
                let mut room_state = OnlineRoomState::new(session, placeholder_view);
                room_state.set_theme(self.local_prefs.theme);
                self.screen = Screen::OnlineRoom(Box::new(room_state));
