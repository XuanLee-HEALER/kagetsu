...
//!
//! 房间收到 [`majo_core::net::protocol::ServerMsg::MpStart`] 后切到此屏. spawn 自己的
//! [`majo_core::net::mp::actor::MpPlayerActor`] + [`majo_core::net::p2p::mp_bridge::spawn_mp_bridge`],
//! 用 [`majo_core::net::session::NetSession::mp_command_tx`] (走 SwarmTransport)
//! 跟 swarm 通信. 入站消息从 [`majo_core::net::session::NetSession::mp_inbound_rx`]
...
use majo_core::net::mp::MpPhase;
use majo_core::net::mp::actor::{MpConfig, MpPlayerHandle, spawn_mp_player};
use majo_core::net::mp::cmd::{MpEvent, MpRoomCmd};
use majo_core::net::p2p::mp_bridge::{MpBridgeHandle, new_inbound_channel, spawn_mp_bridge};
use majo_core::net::p2p::mp_swarm::SwarmTransport;
use majo_core::net::session::NetSession;
use crate::ui::Transition;
use crate::ui::theme::{Theme, ThemeKind};

/// MpStart 参数 — 跟 [`majo_core::net::protocol::ServerMsg::MpStart`] 字段一致.
#[derive(Debug, Clone)]
...
pub struct UiTable {
    ...
    pub own_melds: Vec<(majo_core::mental_poker::wire::WireCallType, Vec<usize>)>,
...
impl ZeroTrustGameState {
    ...
    pub fn advance(&mut self) -> Option<Transition> {
        while let Some(msg) = self.session.try_recv() {
            if let majo_core::net::protocol::ServerMsg::Error { message } = msg {
            ...
            MpEvent::ShouminkanApplied {
                ...
                if player == self.args.own_index {
                    ...
                    if let Some(m) = self.ui_table.own_melds.get_mut(target_meld_idx as usize) {
                        m.0 = majo_core::mental_poker::wire::WireCallType::Kan;
    ...
    pub fn trigger_pon(&self) {
        self.trigger_call(majo_core::mental_poker::wire::WireCallType::Pon, 2);
    }
    ...
    pub fn trigger_chi(&self) {
        self.trigger_call(majo_core::mental_poker::wire::WireCallType::Chi, 2);
    }
    ...
    pub fn trigger_minkan(&self) {
        self.trigger_call(majo_core::mental_poker::wire::WireCallType::Kan, 3);
    }

    /// 通用鸣牌触发 — own_drawn 取前 from_position 张 + last_discard.
    fn trigger_call(
        &self,
        call_type: majo_core::mental_poker::wire::WireCallType,
    ...
    pub fn trigger_shouminkan(&self) {
        ...
        let target_idx = self
            ...
            .own_melds
            .iter()
            .position(|(ct, _)| *ct == majo_core::mental_poker::wire::WireCallType::Pon);
    ...
    pub fn evaluate_yaku(&self) -> Option<YakuScoreSummary> {
        ...
        use majo_core::engine::domain::decompose::decompose;
        use majo_core::engine::domain::meld::{Meld, MeldKind, Seat};
        use majo_core::engine::domain::tile::{Tile, TileIndex};
        use majo_core::engine::domain::yaku::{WinContext, detect_yaku};
        use majo_core::engine::rules::GameRules;
        use majo_core::engine::score::evaluate;
        ...
        let melds: Vec<Meld> = self
            ...
            .filter_map(|(ct, ids)| {
                ...
                let kind = match ct {
                    majo_core::mental_poker::wire::WireCallType::Pon if tiles_vec.len() == 3 => {
                        MeldKind::Pon {
                            tiles: [tiles_vec[0], tiles_vec[1], tiles_vec[2]],
                        }
                    }
                    majo_core::mental_poker::wire::WireCallType::Chi if tiles_vec.len() == 3 => {
                        MeldKind::Chi {
                            tiles: [tiles_vec[0], tiles_vec[1], tiles_vec[2]],
                        }
                    }
                    majo_core::mental_poker::wire::WireCallType::Kan if tiles_vec.len() == 4 => {
                        MeldKind::Minkan {
...
mod tests {
    ...
    async fn four_zerotrust_screens_keygen_and_shuffle_via_swarm() {
        use majo_core::mental_poker::wire::MentalPokerMsg;
        use majo_core::net::p2p::mp_swarm::SwarmCommand;
        use majo_core::net::session::NetSession;
        use libp2p::PeerId;
        ...
        for i in 0..N {
            let (out_tx, _out_rx) = unbounded_channel::<majo_core::net::protocol::ClientMsg>();
            let (_in_tx, in_rx) = unbounded_channel::<majo_core::net::protocol::ServerMsg>();
