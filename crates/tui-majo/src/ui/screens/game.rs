...
use majo_core::ai::dummy::ai_choose_discard;
use majo_core::ai::timeout::default_action_on_timeout;
use majo_core::engine::domain::action::Action;
use majo_core::engine::domain::meld::{MeldKind, Seat};
use majo_core::engine::domain::tile::{Tile, TileIndex};
use majo_core::engine::event::GameEvent;
use majo_core::engine::phase::Phase;
use majo_core::engine::round_state::RoundResult;
use majo_core::engine::rules::GameRules;
use majo_core::engine::score::final_ranking;
use majo_core::game_engine::{CallOptions, GameEngine};
...
impl GameScreenState {
    ...
    fn dev_quick_save(&mut self) {
        match majo_core::dev::savestate::save(&self.engine, "quick") {
            Ok(path) => {
    ...
    fn dev_quick_load(&mut self) {
        match majo_core::dev::savestate::load("quick") {
            Ok(g) => {
                self.engine = g;
    ...
    fn flush_recording_if_any(&mut self) {
        ...
        let rec = majo_core::dev::recorder::RoundRecording {
        ...
        let _ = majo_core::dev::recorder::save(&rec, &filename);
    }
    ...
    fn player(&self) -> &majo_core::engine::player::PlayerState {
    ...
    fn highlight_kind(&self) -> Option<majo_core::engine::domain::tile::TileIndex> {
        if !self.is_player_turn() || self.engine.phase() != Phase::AwaitDiscard {
            return None;
        }
    ...
    fn paint_my_status(&self, buf: &mut Buffer, ox: u16, oy: u16, theme: &Theme) {
        ...
        let waits = if total == 13 {
            majo_core::engine::domain::decompose::tenpai_tiles(
                &majo_core::engine::domain::tile::count_by_kind(&p.hand.closed),
                &p.hand.melds,
            )
        } else if total == 14 {
            // 摸完未切: 把 last_drawn 排除后剩 13 张算 tenpai.
            if let Some(drawn) = p.last_drawn {
                let mut counts = majo_core::engine::domain::tile::count_by_kind(&p.hand.closed);
                counts[drawn.kind.0 as usize] = counts[drawn.kind.0 as usize].saturating_sub(1);
                majo_core::engine::domain::decompose::tenpai_tiles(&counts, &p.hand.melds)
            } else {
...
fn riichi_index_in_river(p: &majo_core::engine::player::PlayerState) -> Option<usize> {
    p.riichi_river_idx
}
