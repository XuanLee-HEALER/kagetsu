...
async fn main() -> Result<()> {
    ...
    // majo-core link verification — 把它装进一个 _ binding 即可让 cargo
    ...
    info!(%addr, "web-majo listening on http://{}", addr);
...
/// 2. 否则取 `CARGO_MANIFEST_DIR/static` (开发时 `cargo run -p web-majo` 即可)
