...
复用 `majo_core::net` (libp2p) 加入同一个 P2P 网络。
...
### 下一阶段: svelte 翻译
- [ ] 在 `crates/web-majo/frontend/` 起 Vite + Svelte 工程
...
- [ ] tracking: 见 task list 中 "把 web-majo 原型转 svelte"

### 再之后: 真业务
- [ ] WebSocket endpoint (`/ws`): browser ↔ backend 协议
- [ ] backend 内嵌 `majo_core::net::session::NetSession`,
      跟 libp2p P2P 网通信
- [ ] HTTP REST: 大厅 / 房间列表 / 历史回放
- [ ] auth / session: 用 `majo_core::config::LocalPrefs` 持久身份
