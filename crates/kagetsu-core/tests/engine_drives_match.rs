...
use kagetsu_core::engine::match_state::{MatchState, match_apply};
use kagetsu_core::engine::op::{AtomicOp, OpError};
use kagetsu_core::engine::round_state::{
    RoundState, init_round, legal_ops, round_apply, summarize_round,
};
use kagetsu_core::engine::rules::{GameRules, LengthRule};
...
fn decide(state: &RoundState) -> AtomicOp {
    match state {
        ...
        RoundState::AwaitCalls(_) => {
            let ops = legal_ops(state);
            // 任何家能荣和就 Ron.
            for who in kagetsu_core::engine::domain::meld::Seat::ALL {
...
fn engine_drives_one_round_to_end() {
    ...
    match outcome {
        kagetsu_core::engine::match_state::RoundOutcome::Win { payments, .. } => {
            assert!(!payments.is_empty(), "Win 必有 payments");
        }
        kagetsu_core::engine::match_state::RoundOutcome::Ryuukyoku { .. } => {}
    }
}
...
fn round_apply_rejects_illegal_op_for_phase() {
    ...
    let dummy_tile = kagetsu_core::engine::domain::tile::Tile {
        kind: kagetsu_core::engine::domain::tile::TileIndex(0),
        red: false,
        id: 0,
    };
