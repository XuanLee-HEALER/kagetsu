...
use kagetsu_core::engine::domain::decompose::{Decomposition, decompose};
use kagetsu_core::engine::domain::meld::{Meld, MeldKind, Seat};
use kagetsu_core::engine::domain::tile::{Tile, TileIndex, count_by_kind};
use kagetsu_core::engine::domain::yaku::WinContext;
...
fn verify_hora(state: &ReplayState, log: &KyokuLog, config: &GameRules) -> Vec<ReplayDiff> {
    ...
    // 取分数最高的分解
    let mut best: Option<kagetsu_core::engine::score::ScoreResult> = None;
    for decomp in &decomps {
        ...
        if let Some(res) = kagetsu_core::engine::score::evaluate(&ctx, &state.melds[widx])
            && best
                .as_ref()
