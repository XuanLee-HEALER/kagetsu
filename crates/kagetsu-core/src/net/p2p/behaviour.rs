...
/// 协议名: client → server.
pub const PROTOCOL_C2S: StreamProtocol = StreamProtocol::new("/kagetsu/c2s/1");
/// 协议名: server → client.
pub const PROTOCOL_S2C: StreamProtocol = StreamProtocol::new("/kagetsu/s2c/1");
/// 协议名: ZeroTrust 模式 P2P 对等通信 (M5.B.8). 用于 unicast (DrawShareRequest /
/// Response, ConcealedKanReveal). broadcast 走 gossipsub mp_topic.
pub const PROTOCOL_MP: StreamProtocol = StreamProtocol::new("/kagetsu/mp/1");
/// identify 协议 agent_version 前缀, 后跟房间 metadata k=v;k=v 字符串.
pub const AGENT_PREFIX: &str = "kagetsu/";
/// gossipsub topic: 在线房间广播.
pub const LOBBY_TOPIC: &str = "kagetsu/lobby/v1";
/// gossipsub topic 前缀: ZeroTrust 模式 mental poker broadcast (M5.B.8).
/// 完整 topic = `kagetsu/mp/<room_id>/v1`, 4 玩家在 MpStart 后订阅.
pub const MP_TOPIC_PREFIX: &str = "kagetsu/mp/";
...
pub const RELAYS_TOPIC: &str = "kagetsu/relays/v1";
    ...
    pub fn new(
        ...
        Self {
            identify: identify::Behaviour::new(
                identify::Config::new("/kagetsu/id/1".into(), local_pubkey)
...
/// 计算 ZeroTrust 房间的 gossipsub mp topic 名: `kagetsu/mp/<room_id>/v1`.
